...
function createPutExtraChecks(methodName) {
  return function extraChecks(options) {
    ...
    if (!options.token.startsWith("vercel_blob_client_")) {
      throw new (0, _chunkWOCAUB3Mcjs.BlobError)(`${methodName} must be called with a client token`);
    }
    if (
...
var upload = _chunkWOCAUB3Mcjs.createPutMethod.call(void 0, {
  allowedOptions: ["contentType"],
  extraChecks(options) {
    if (typeof window === "undefined") {
      throw new (0, _chunkWOCAUB3Mcjs.BlobError)(
...
async function handleUpload({
  ...
  var _a, _b, _c, _d;
  const resolvedToken = _chunkWOCAUB3Mcjs.getTokenFromOptionsOrEnv.call(void 0, { token });
  ...
  switch (type) {
    ...
    case "blob.upload-completed": {
      ...
      if (!signature) {
        throw new (0, _chunkWOCAUB3Mcjs.BlobError)("Missing callback signature");
      }
      ...
      if (!isVerified) {
        throw new (0, _chunkWOCAUB3Mcjs.BlobError)("Invalid callback signature");
      }
      await onUploadCompleted(body.payload);
      return { type, response: "ok" };
    }
    default:
      throw new (0, _chunkWOCAUB3Mcjs.BlobError)("Invalid event type");
  }
}
async function retrieveClientToken(options) {
  ...
  if (!res.ok) {
    throw new (0, _chunkWOCAUB3Mcjs.BlobError)("Failed to  retrieve the client token");
  }
  try {
...
async function generateClientTokenFromReadWriteToken({
  ...
  const readWriteToken = _chunkWOCAUB3Mcjs.getTokenFromOptionsOrEnv.call(void 0, { token });
  ...
  if (!securedKey) {
    throw new (0, _chunkWOCAUB3Mcjs.BlobError)("Unable to sign client token");
  }
